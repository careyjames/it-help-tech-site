...
- Topbar surface treatment is stratified by capability, not just by width: BASE rule (all viewports) sets `background: var(--c1)` so the topbar is opaque and exactly matches body bg — by construction no chroma/luminance step exists at the topbar/body junction, so no seam can render. DESKTOP rule (`@media (min-width: 961px) and (hover: hover) and (pointer: fine)`) re-enables the Apple glass aesthetic + 38% gold WCAG 1.4.11 hairline, but mixes from `--c1` (not `--surface-charcoal`) so even in translucent state the topbar reads continuous with the page. TOUCH override uses **`@media (any-pointer: coarse), (any-hover: none)`** (NOT primary-input `pointer: coarse`/`hover: none`) — `any-pointer`/`any-hover` evaluate ANY available input device, which correctly catches iPadOS Safari even when it deliberately reports `pointer: fine` + `hover: hover` to masquerade as a desktop browser (a documented iPadOS 13+ behavior). The primary-input variant fails silently on iPad Pro in landscape with a Magic Keyboard, leaving the desktop glass + gold hairline visible as a horizontal band. Never tint the topbar with a token that doesn't match `--c1`; the seam will return. Never gate the touch override on primary-input queries; iPad will slip through.
